...
func GetBalance(w http.ResponseWriter, r *http.Request, storage *storage.Storage, secretKey []byte) {
...
func Withdraw(w http.ResponseWriter, r *http.Request, storage *storage.Storage, secretKey []byte) {
...
func GetAllWithdrawals(w http.ResponseWriter, r *http.Request, storage *storage.Storage, secretKey []byte) {
	headerContentType := r.Header.Get("Content-Length")
	if len(headerContentType) != 0 {
		messageResponse(w, "Content-Length is not equal 0", "application/json", http.StatusBadRequest)
		return
