...
func (pdb *PostgresDB) CreateOrder(order *models.Order) error {
	order.Status = "NEW"
	_, err := pdb.pool.Exec(context.Background(), "INSERT INTO orders (user_id, order_num, status, withdrawn_at) VALUES ($1, $2, $3, NULL) ON CONFLICT (order_num) DO NOTHING", order.UserID, order.OrderNum, order.Status)
...
func (pdb *PostgresDB) ReadAllWithdrawals(userID uuid.UUID) ([]*models.WithdrawDB, error) {
	...
	rows, err := pdb.pool.Query(context.Background(), "SELECT order_num, withdrawal, created_at FROM orders WHERE (user_id=$1 AND withdrawn_at IS NOT NULL) ORDER BY created_at", userID)
	if err != nil {
		log.Println(err)
...
func (pdb *PostgresDB) ReadOrdersForProcessing() ([]string, error) {
	...
	rows, err := pdb.pool.Query(context.Background(), "SELECT order_num FROM orders WHERE (status IN ('NEW','PROCESSING') AND withdrawn_at IS NULL) ORDER BY created_at")
	if err != nil {
		log.Println(err)
		return orders, err
	}
...
func (pdb *PostgresDB) UpdateOrder(order models.ProcessingOrder) {
	...
	if order.Accrual != nil {
		query = fmt.Sprintf("UPDATE orders SET status = '%s', accrual = '%f' WHERE (order_num = '%s' AND withdrawn_at IS NULL);", order.Status, order.Accrual.(float64), order.OrderNum)
	} else {
		query = fmt.Sprintf("UPDATE orders SET status = '%s' WHERE (order_num = '%s' AND withdrawn_at IS NULL);", order.Status, order.OrderNum)
	}
